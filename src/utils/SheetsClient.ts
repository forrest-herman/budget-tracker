...
export default class SheetsClient {
  ...
  ): Promise<ExpenseTransaction[] | IncomeTransaction[]> {
      ...
      requestBody: {
        values: transactions.map((transaction) => [
          ...
          isExpense
            ...
            : transaction.pay_period_start
            ? transaction.pay_period_start.toLocaleDateString()
            : '',
          isExpense
            ? transaction.unit_count || ''
            : transaction.pay_period_end
            ? transaction.pay_period_end.toLocaleDateString()
            : '',
  ...
  async getTransactions(options?: {
    ...
    if (options?.start_date && options?.end_date) {
      ...
      query += ` AND date '${options.start_date.toISOString().substring(0, 10)}' <= ${this.columnIds.date} 
                and ${this.columnIds.date} <= date '${options.end_date.toISOString().substring(0, 10)}'`;
  ...
  async getTotalSpending(options?: { startDate?: Date; endDate?: Date }): Promise<number> {
    ...
    if (options?.startDate && options?.endDate) {
      ...
      query += ` where date '${options.startDate.toISOString().substring(0, 10)}' <= ${this.columnIds.date} 
                and ${this.columnIds.date} <= date '${options.endDate.toISOString().substring(0, 10)}'`;
  ...
  async sortSheet(
    sheetTitle: string,
    options?: { sortColumnLabel?: string; sortColumnIndex?: number; direction?: 'ASCENDING' | 'DESCENDING' },
  ) {
    const spreadsheetProperties = await this.sheets.spreadsheets.get({
      spreadsheetId: this.spreadsheet_id,
    });

    const sheetProperties = spreadsheetProperties?.data?.sheets
      ?.filter((sheet) => sheet?.properties?.title === sheetTitle)
      ?.at(0)?.properties;
